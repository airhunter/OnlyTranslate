...
阅读电子书时，在 Popup 底部选择「电子书（Beta）」，点击「导入 EPUB」并选择一本本地无 DRM 的 EPUB。导入后可从 Popup 书架继续阅读，也可以进入完整书架管理图书。
...
- **划词、悬停和输入框翻译**：选中内容、指向文字，或在网页输入框中使用指定字符，即可快速翻译。
- **视频字幕翻译**：自动获取支持站点的原字幕，结合上下文分段翻译并显示双语字幕；播放调度与本地缓存减少等待和重复请求。
- **电子书翻译（Beta）**：导入本地 EPUB，在滚动阅读时自动翻译当前章节，并长期保存书架、阅读进度和位置书签。
...
从 Popup 底部进入「电子书」，可以直接导入 EPUB、查看最近阅读和继续打开图书。完整阅读器采用纵向滚动模式，进入章节后自动翻译当前章，并支持双语 / 仅译文、目录跳转、主题、字号、行距和当前位置书签。

- 第一版仅支持本地无 DRM 的 EPUB，不支持 PDF、MOBI 或受 DRM 保护的图书。
- 图书、阅读进度和书签保存在当前浏览器配置中，不提供云同步、导出、笔记或 AI 分析。
- EPUB 文件本身不会上传；开始翻译后，章节文本会发送给当前选择的翻译服务。
- 移除图书会同时删除对应的阅读进度和书签；卸载扩展或清除扩展数据也会丢失本地书架。
...
- 导入的 EPUB、阅读进度和书签保存在浏览器本地；只有需要翻译的章节文本会发送给所选翻译服务。
